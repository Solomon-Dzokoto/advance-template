...
export const presentationService = {
    createPresentation: async (title: string): Promise<Presentation> => {
        try {
            ...
            throw new PresentationError("Could not create presentation. Please try again.");
        }
    },
    ...
    getPresentation: async (id: string): Promise<Presentation | undefined> => {
        try {
            ...
            throw new PresentationError("Could not load presentation. Please try again.");
        }
    },

    updatePresentation: async (id: string, slides: Slide[]): Promise<Presentation | undefined> => {
        try {
            const result = await withRetry(() => backend.update_presentation(id, slides));
            return result.length > 0 ? result[0] : undefined;
        } catch (error) {
            console.error("Failed to update presentation:", error);
            throw new PresentationError("Could not save presentation. Please try again.");
        }
    },

    generateSlideSuggestions: async (topic: string): Promise<string[]> => {
        if (!topic.trim()) {
            throw new PresentationError("Please enter a topic for your presentation.");
        }
